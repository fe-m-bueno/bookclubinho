...
class Round(CreatedAtMixin, Base):
    ...
    nominations: Mapped[list["RoundNomination"]] = relationship(
        back_populates="round", lazy="raise"
    )
...
class RoundNomination(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "round_id", "user_id", "book_id", name="uq_round_nominations_round_user_book"
        ),
        CheckConstraint(
    ...
    votes: Mapped[list["RoundVote"]] = relationship(
        back_populates="nomination", lazy="raise"
    )
...
class RoundVote(Base):
    ...
    __table_args__ = (
        UniqueConstraint("round_id", "user_id", name="uq_round_votes_round_user"),
    )
    ...
    user: Mapped["User"] = relationship(lazy="raise")  # noqa: F821
    nomination: Mapped["RoundNomination"] = relationship(
        back_populates="votes", lazy="raise"
    )
